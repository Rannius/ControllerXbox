...
GFN and Boosteroid badges can be enabled or disabled independently in the quick-access panel. The four notification categories can also be toggled separately. A persistent, change-based background check notifies the user once when a library game becomes available on GeForce NOW or Boosteroid, enters Boosteroid maintenance, or when a new stable ControllerXbox release is available. The first check creates a baseline and does not announce the entire existing catalog.
