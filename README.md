...
In **Beállítások**, **Felületfrissítés ébresztés után** enables the same reload three seconds after a suspend/resume pair observed by the running plugin. It defaults to off; test the manual action before enabling it. It works with the plugin panel closed, skips locked screens (retry manually after unlocking), and cancels pending work when disabled, suspended again, or unloaded. Re-registering the plugin after a UI reload does not trigger another refresh, preventing a reload loop. Unsupported APIs are reported without escalating to a Steam restart. A frozen JS context cannot run this feature, and restarting the UI may not resolve faults in native Steam Input or the controller driver.

Version 1.0.47 fixes the disabled automatic-refresh switch on Steam clients that removed the legacy `System` suspend/resume methods. It also supports `User` suspend/resume progress events and Steam's `SleepManager` wake notification. Progress updates are filtered so a UI reload is requested only after resume completes, and replayed completed states cannot arm another reload. A throwing legacy API falls back to the newer events. If no compatible event pair is available, the panel identifies the missing notification; an already-enabled setting can always be switched off.
