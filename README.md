...
The home panel offers **Steam felület újratöltése**, which calls `SteamClient.Browser.RestartJSContext()` to restart Steam's shared JavaScript UI context, the same API used by [Decky's reinjection helper](https://github.com/SteamDeckHomebrew/decky-loader/blob/f75dc1df134daf53b1ec02c351b12313c1696a7e/backend/decky_loader/helpers.py). The UI and plugin frontends temporarily disappear and reload. This requests no Steam process restart, desktop switch, game termination, driver reset, or controller mapping change. It is an experimental workaround; neither recovery of the physical buttons nor uninterrupted gameplay has been verified on hardware. Save before the first manual test. The native API returns no completion acknowledgement, so the plugin reports only that a reload was requested.
...
The manual action is inside Decky's quick-access panel; this version has no independent touch shortcut if that panel cannot be opened. On-device validation: while the menu buttons still work, save the game, open the plugin, and test the manual reload. Verify game continuity and Decky returning. Then enable automatic refresh before suspending and check both physical buttons after resume. Also check PIN lock behavior, disabling the feature, and plugin unload before a pending refresh. Recovery of the reported button failure still needs testing on the affected Deck. Local checks: `pnpm check`, `pnpm test`, `python -m unittest discover -s tests -p 'test_*.py'`, and `pnpm build`.
